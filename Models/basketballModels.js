...
function getTeamURL(counter){
    ...
    return `https://site.web.api.espn.com/apis/site/v2/sports/basketball/nba/teams/${counter}/schedule?seasontype=2`;
}

// Get a singular team's info
function getTeam(teamid){
    const sql = 'SELECT * FROM Basketball WHERE teamid=@teamid';
...
function getTeamAtWeek(teamid){
    const sql = 'SELECT * FROM Basketball WHERE teamid=@teamid AND week=@week';
    const stmt = db.prepare(sql);
    const teamAtWeek = stmt.all({teamid}, {week});
...
function getTeamsAtWeek(week) {
    const sql = 'SELECT * FROM Basketball WHERE week=@week';
    const stmt = db.prepare(sql);
...
function getOpponents(teamid) {
    const sql = 'SELECT * FROM Basketball WHERE opponentid=@teamid';
    const stmt = db.prepare(sql);
    const team = stmt.all({teamid});
...
async function populateDatabase() {
            ...
            if (i != 11 && i != 21 && i != 28 && i != 29){
                ...
                for (let j = 0; j < 16; j++){
                    ...
                    if (teamWeek?.competitors[0]?.id == currentTeam?.team?.id){
                        ...
                        const sql = 'INSERT OR IGNORE INTO Basketball VALUES (@teamid, @teamname, @logo, @record, @week, @score, @winStatus, @visitStatus, @opponentid)'
                        const stmt = db.prepare(sql);
    ...
    console.log("Basketball Database Updated");
}
